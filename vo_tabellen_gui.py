...
def build_table1_workbook(raw_path, layout_path, internal_layout: bool):
    ...
    # Sonderfall: Jahrestabelle (_g) soll die Eingangsdatei vollständig übernehmen (inkl. Spalten J/K),
    # zusätzlich Markierung (gelb), wenn in Spalte G der Wert 1 oder 2 steht.
    # Erkennung über den Periodentext oberhalb des Tabellenkopfs (bei Jahr nur die Jahreszahl, z.B. "2025").
    if (not internal_layout) and period_text and re.fullmatch(r"\d{4}", str(period_text).strip() or ""):
        year = str(period_text).strip()
        wb_out = openpyxl.load_workbook(raw_path)  # mit Layout/Format/Spaltenbreiten
        ws_out = wb_out.active

        # Periodentext wie gefordert: "Jahr 2025"
        ws_out["A3"].value = f"Jahr {year}"

        # Gelb markieren: nur die Zelle in Spalte G, wenn Wert 1 oder 2
        yellow = PatternFill(start_color="FFFF00", end_color="FFFF00", fill_type="solid")
        for r in range(1, ws_out.max_row + 1):
            v = ws_out.cell(row=r, column=7).value
            try:
                iv = int(str(v).strip())
            except Exception:
                iv = None
            if iv in (1, 2):
                ws_out.cell(row=r, column=7).fill = yellow
